...
int setBitTo1OnEvery3thPosition(int number) {
...
int main(void) {
    ...
    int newNumber1 = setBitTo1OnEvery3thPosition(number1);
    
    int2bin(newNumber1, buffer, BUF_SIZE - 1);
    printf("number %d set bit on every 3th position to 1 = %d == %s\n", number1, newNumber1, buffer);
	
    int newNumber2 = setBitTo1OnEvery3thPosition(number2);
