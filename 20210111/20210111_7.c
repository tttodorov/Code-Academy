...
int setBitTo0OnEvenPosition(int number) {
...
int main(void) {
    ...
    int newNumber1 = setBitTo0OnEvenPosition(number1);
    
    int2bin(newNumber1, buffer, BUF_SIZE - 1);
    printf("number %d set bit on even position to 0 = %d == %s\n", number1, newNumber1, buffer);
	
    int newNumber2 = setBitTo0OnEvenPosition(number2);
